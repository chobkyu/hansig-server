...
describe("post /owner", function () {
  describe("success", () => {
    ...
    it("201로 응답한다.", (done) => {
      request(app).post("/owner").send(testData).expect(201).end(done);
    });
  });
  ...
  describe("fail..", () => {
    it("사업자가 아닐 경우 401로 응답", (done) => {
      ...
      request(app).post("/owner").send(testData).expect(401).end(done);
    });
    ...
    it("아이디가 중복일 경우 409로 응답", (done) => {
      ...
      request(app).post("/owner").send(testData).expect(409).end(done);
    });
    ...
    it("입력값이 누락되었을 경우 400으로 응답", (done) => {
      ...
      request(app).post("/owner").send(testData).expect(400).end(done);
    });
...
describe("post /owner/login ", function () {
  ...
  describe("fail", () => {
    it("입력이 잘못 되었을 시 400으로 응답한다.", (done) => {
      ...
      request(app).post("/owner/login").send(body).expect(400).end(done);
    });
    ...
    it("사업자가 아닐 시 401로 응답한다.", (done) => {
      ...
      request(app).post("/owner/login").send(body).expect(401).end(done);
    });
  });
});
...
describe("post /owner/menu", function () {
  describe("성공 시 ", () => {
    ...
    it("성공 시 201로 응답한다.", (done) => {
      request(app).post("/owner/menu").send(menu).expect(201).end(done);
    });
  });
  ...
  describe("실패 시", () => {
    it("입력 값이 잘못 되었을 경우 400으로 응답한다.", (done) => {
      ...
      request(app).post("/owner/menu").send(menu).expect(400).end(done);
    });
...
describe("delete /owner/menu/:id", function () {
  ...
  describe("실패 시", () => {
    ...
    it("로그인이 안되어 있을 시 401로 응답", (done) => {
      request(app).delete("/owner/menu/1").expect(400).end(done);
    });
  });
});
