...
const refresh = async (req: Request, res: Response) => {
  ...
  if (req.headers.authorization && req.headers.refresh) {
    ...
    if (authResult.success === false && authResult.msg === 'jwt expired') {
      // 1. access token이 만료되고, refresh token도 만료 되거나 db와 일치하지
      // 않는경우 => 새로 로그인해야합니다.
      if (refreshResult === false) {
        ...
        // 2. access token이 만료되고, refresh token은 만료되지 않은 경우 =>
        ...
        res.status(200).send({
          // 새로 발급한 access token과 원래 있던 refresh token 모두
          // 클라이언트에게 반환합니다.
          success : true,
          data : {access : newAccessToken, refresh : refreshToken},
        });
      }
    } else {
